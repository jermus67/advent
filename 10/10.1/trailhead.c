...
int** get_trailhead_nine_heights(int** map, int rows, int cols, int x, int y, int height) {
    ...
    if (x >= 0 && x < cols && y >= 0 && y < rows) {
        if (map[y][x] == height) {
             ...
             if (height == 9) {
                ...
                int **score_up = get_trailhead_nine_heights(map, rows, cols, x, y - 1, height + 1);
                int **score_down = get_trailhead_nine_heights(map, rows, cols, x, y + 1, height + 1);
                int **score_left = get_trailhead_nine_heights(map, rows, cols, x - 1, y, height + 1);
                int **score_right = get_trailhead_nine_heights(map, rows, cols, x + 1, y, height + 1);
...
int main() {
    ...
    for (int i = 0; i < cols; i++) {
            if (map[j][i] == 0) {
                ...
                int **nine_heights = get_trailhead_nine_heights(map, rows, cols, i, j, 0);
