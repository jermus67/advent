...
void validate_rule(struct update* update, struct page_ordering_rule* rule) {
    ...
    if (before_page_number_index != -1 && after_page_number_index != -1) {
        if (before_page_number_index > after_page_number_index) {
            int before_page_number = update->page_numbers->page_numbers[before_page_number_index];
            int after_page_number = update->page_numbers->page_numbers[after_page_number_index];
            update->page_numbers->page_numbers[before_page_number_index] = after_page_number;
            update->page_numbers->page_numbers[after_page_number_index] = before_page_number;
            update->valid = 0;
        }
    }
}

void reorder_pages(struct update* update, struct page_ordering_rule_sequence* rules) {
    while(!update->valid) {
        update->valid = 1;
...
int main() {
    ...
    for (size_t i = 0; i < print_queue->updates->size; i++) {
        ...
        if (!update->valid) {
            ...
            printf(" -> %d\n", update->middle_page);
            total_reordered_middle_pages += update->middle_page;
        }
    }

    printf("Total reordered middle pages: %d\n", total_reordered_middle_pages);
