...
typedef long long (*operator)(long long, const char* );
...
long long add_operator(long long a, const char* b) {
    return a + atoll(b);
}

long long multiply_operator(long long a, const char* b) {
    return a * atoll(b);
}

long long concat_operator(long long a, const char* b) {
    char result[BUFSIZ];
    sprintf(result, "%lld", a);
    strcat(result, b);
    return atoll(result);
}
...
void equation_add_number(struct equation *equation, const char* number) {
    if (equation->numbers_size == equation->numbers_capacity) {
        ...
        equation->numbers = realloc(equation->numbers, equation->numbers_capacity * sizeof(char *));
    }
    equation->numbers[equation->numbers_size++] = strdup(number);
}
...
long long equation_calculate(struct equation *equation) {
    ...
    long long result = atoll(equation->numbers[0]);
...
void equations_read_from_stdin(struct equations *equations) {
    ...
    while(fgets(buffer, BUFSIZ, stdin) != NULL) {
        ...
        struct equation *equation = equation_create(atoll(token));
        while ((token = strtok(NULL, " \n")) != NULL) {
            equation_add_number(equation, token);
        }
